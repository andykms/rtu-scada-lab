...
@Component({
  ...
  styleUrls: ['../shared/create-block-form.css', './modbus-tcp.component.css'],
  standalone: true,
...
export class ModbusTcpComponent extends LanguageProvider {
  private readonly context = injectDialogContext<
    ...
    ICreateBlockDialogData<IModbusTcpBlock>
  >();
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    tcpPort: [502, [Validators.required, Validators.min(1), Validators.max(65535)]],
    isFullInput: [false],
    fullCommand: [''],
  ...
  private applyInitialState(): void {
    ...
    this.form.patchValue({
      ...
      tcpHost: block.tcpHost,
      tcpPort: block.tcpPort,
      slaveId: cmd.slaveId,
  ...
  private submit(): void {
    ...
    this.context.completeWith({
      block: {
        ...
        tcpHost: isDemoMode ? '127.0.0.1' : raw.tcpHost.trim(),
        tcpPort: isDemoMode ? 502 : Number(raw.tcpPort),
        command,
